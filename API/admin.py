...
def end_trip():
    ...
    try:
        sql = 'BEGIN;' \
                ...
                'UPDATE bus SET status = 1 FROM trip WHERE trip.bus_id = bus.id AND trip.id = %s; ' \
                'UPDATE bus_seat SET status = 1 FROM trip WHERE trip.bus_id = bus_seat.bus_id AND trip.id = %s; ' \
                'COMMIT;'
